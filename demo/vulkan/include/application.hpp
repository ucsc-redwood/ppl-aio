...
class ApplicationBase {
 ...
 protected:
  ...
  VkDescriptorSetLayoutBinding build_layout_binding(
  ...
  void create_descriptor_set_layout(
    ...
    *pCreateInfo = descriptorLayout;
    vkCreateDescriptorSetLayout(
        singleton.device, pCreateInfo, nullptr, pSetLayout);
  }

  VkPipelineLayoutCreateInfo init_pipeline_layout(
  ...
  VkPushConstantRange init_push_constant(VkShaderStageFlags stageFlags,
                                         uint32_t offset,
                                         uint32_t size) {
  ...
  void add_push_constant(VkPipelineLayoutCreateInfo* pipelineLayoutCreateInfo,
                         VkPushConstantRange* push_constant,
  ...
  void allocate_command_buffer(uint32_t commandBufferCount) {
    ...
    vkAllocateCommandBuffers(
        singleton.device, &cmdBufAllocateInfo, &commandBuffer);
  }

  void allocate_descriptor_sets(uint32_t descriptorSetCount,
  ...
  VkWriteDescriptorSet create_descriptor_write(
  ...
  VkPipelineShaderStageCreateInfo load_shader(const std::string shader_name,
    ...
    shaderStage.module = tools::loadShader((shadersPath + shader_name).c_str(),
                                           singleton.device);
  ...
  void create_pipeline(VkPipelineShaderStageCreateInfo* shaderStage,
    ...
    vkCreatePipelineCache(
        singleton.device, &pipelineCacheCreateInfo, nullptr, &pipelineCache);
    // Create pipeline
    ...
    vkCreateComputePipelines(singleton.device,
                             pipelineCache,
                             1,
  ...
  void create_descriptor_pool(std::vector<VkDescriptorPoolSize> poolSizes,
                              uint32_t maxSets) {
    ...
    vkCreateDescriptorPool(
        singleton.device, &descriptorPoolInfo, nullptr, &descriptorPool);
  }

  void create_storage_buffer(const VkDeviceSize bufferSize,
                             void* data,
    ...
    singleton.createBuffer(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT |
  ...
  void create_shared_storage_buffer(const VkDeviceSize bufferSize,
                                    VkBuffer* device_buffer,
                                    VkDeviceMemory* device_memory,
                                    void* data,
                                    void** mapped) {
    ...
    // void* mapped_temp;
    if (vkMapMemory(
            singleton.device, *device_memory, 0, bufferSize, 0, mapped) ==
  ...
  void create_shared_empty_storage_buffer(const VkDeviceSize bufferSize,
                                          VkBuffer* device_buffer,
                                          VkDeviceMemory* device_memory,
                                          void** mapped) {
    // Copy input data to VRAM using a staging buffer
  ...
  VkBufferMemoryBarrier create_buffer_barrier(VkBuffer* buffer,
